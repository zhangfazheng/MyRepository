...
+ (NSString *)dbPathWithDirectoryName:(NSString *)directoryName
{
    ...
    if (directoryName == nil || directoryName.length == 0) {
        docsdir = [docsdir stringByAppendingPathComponent:@"MYBD"];
    } else {
...
- (BOOL)changeDBWithDirectoryName:(NSString *)directoryName
{
    ...
    _instance.dbQueue = [[FMDatabaseQueue alloc] initWithPath:[FMDBManager dbPathWithDirectoryName:directoryName]];
    ...
    if (numClasses >0 )
    {
        ...
        for (int i = 0; i < numClasses; i++) {
            if (class_getSuperclass(classes[i]) == [FMDBManager class]){
                id class = classes[i];
